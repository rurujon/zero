...
function BbsUpdate() {
    ...
    const updateBbs = async () => {
        const req = {
            memId: localStorage.getItem("memId"), // 로그인한 사용자 ID를 localStorage에서 가져옴
            title: title,
            content: content
        };

        await axios.patch(`http://localhost:8080/bbs/${bbs.seq}`, req, { headers: headers })
            .then((resp) => {
            ...
            <table className="table">
                <tbody>
                    <tr>
                        ...
                        <td>
                            <input type="text" className="form-control" value={bbs.memId} size="50px" readOnly />
                        </td>
                    </tr>
